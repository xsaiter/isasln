...
aho_corasick_t::search(const std::string &text) const {
  ...
  for (std::size_t i = 0; i < len; ++i) {
    ...
    while (loop) {
      loop = false;
      node_ptr_s next = next_state(current, c);
      if (next) {
        current = next;
  ...
  for (const std::string &pattern : patterns_) {
    ...
    for (const char c : pattern) {
      ...
      if (next) {
        ...
        node_ptr_s node = make_node(c, n++, current);
...
void aho_corasick_t::build_failure() {
  std::queue<node_ptr_s> q;
  bool first = true;

  node_ptr_s node = root_;
  q.push(node);
  ...
  while (!q.empty()) {
    ...
    for (node_ptr_s child : node->children) {
      q.push(child);
      if (first) {
        child->failure = root_;
        first = false;
      } else {
        std::vector<node_ptr_s> path;
...
aho_corasick_t::node_ptr_s aho_corasick_t::next_state(node_ptr_s node,
                                                      char c) const {
  auto found = std::find_if(node->children.begin(), node->children.end(),
                            [&](node_ptr_s n) { return n->c == c; });
...
aho_corasick_t::node_ptr_s
aho_corasick_t::make_node(char c, unsigned int n,
                          aho_corasick_t::node_ptr_s parent) const {
  node_ptr_s node = std::make_shared<node_t>();
  node->c = c;
  node->n = n;
  node->parent = parent;
  return node;
...
void aho_corasick_t::append(std::vector<result_t> &result, node_ptr_s node,
